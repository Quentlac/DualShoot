...
void draw(){
  ...
  if(version_client == version_serveur){
    ...
    if(open_nav == 0){
      open_nav = 1;
      link("http://192.168.1.12/DualShoot/File/dualshoot"+str(version_serveur)+".zip");
    }
...
void setAnglePers(){
  //On va viser le joueur le plus proche de nous
  float dist_min = 10000;
  int perso_proche = -1;
  for(int i = 0; i < nb_joueur;i++){
    float dist = sqrt(abs(xPers - joueur[i].getX())*abs(xPers - joueur[i].getX())+abs(yPers - joueur[i].getY())*abs(yPers - joueur[i].getY()));
    if(dist < dist_min){
      dist_min = dist;
      perso_proche = i;
    }
  }
  println(perso_proche);
  if(perso_proche != -1){
    float x = xPers+joueur[perso_proche].getX()-(width/2);
    float y = yPers+joueur[perso_proche].getY()-(height/2);
    
    
    if(y == 0)y = 1;
    
    if(joueur[perso_proche].getY() < height/2){
